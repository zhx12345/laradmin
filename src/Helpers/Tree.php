...
use App\Helpers\不能选择项;
use App\Helpers\当前选择项;
use App\Helpers\间歇;
use App\Helpers\顶部id;
...
class Tree
{
    ...
    public static $childrenname = 'children';
     ...
     * 其他参数
     *
     * @param  [categories] $config [description] option array('key'=>'value','key1'=>'value1');
     ...
     *
     * @param  [categories]  $data  [description] 当前栏目id
     * @param  int  $pid [description]当前栏目id
        ...
        if (is_array($child)) {
            foreach ($child as $k => $v) {
                ...
                if (self::$isGetUpperIdAll === true) {
                    //$v['supper_idall']=self::get_parentid_all($v[self::$id]);
                    $v['supper_idall'] = implode('_', self::get_parentid_all($v[self::$id]));
                }
                //如果存在模型
                $arr[] = $v;
     ...
     *
     * @param  [categories] $curid [description] 当前id
     ...
     * 生成select
     *
     * @Desc:  $noselectedpid=array('parentid',array(0,1));及栏目id 不能选择父id值为0和1的栏目
     *
     * @param $topid  顶部id
     * @param $selectedid 当前选择项
     * @param $noselectedpid 不能选择项
     * @param $adds 间歇
     *
     * @author: zhx
     *
     * @Time: 2023/8/15 10:47
     */
    ...
    public static function get_childall_data($topid = 0, $sort = [], $child = []): array
    {
        foreach (self::$arr as $k => $v) {
            ...
            if ($v[self::$pid] == $topid) {
                ...
                $v[self::$childrenname] = $child_arr ?? [];
                if (! empty($sort) && ! empty($v['child'])) {
                    $v[self::$childrenname] = self::data_sort($v['child'], $sort[0], $sort[1]);
                    $v[self::$childrenname] = array_values($v['child']);
                }
                $child[] = $v;
            }
        }
